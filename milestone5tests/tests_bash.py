...
def _test_commands_exist(comment_file_path, student_dir, command_wait=0.01):
...
def _test_fictional_fail(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    write(p,"clea")
    sleep(command_wait)
    write(p,"sleeep 0.0001")
    sleep(command_wait)
    error1 = read_stderr(p)
    error2 = read_stderr(p)
    error3 = read_stderr(p)
    if "ERROR" in error1 and "ERROR" in error2 and "ERROR" in error3:
...
def _test_tail(comment_file_path, student_dir, command_wait=0.01, max_length=20):
...
def _test_pipe_tail(comment_file_path, student_dir, command_wait=0.01, max_length=20):
...
def _test_pipe_head(comment_file_path, student_dir, command_wait=0.01, max_length=20):
...
def test_bash_suite(comment_file_path, student_dir):
  ...
  start_with_timeout(_test_commands_exist, comment_file_path, student_dir, timeoutFeedback="OK\n")
