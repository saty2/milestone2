...
def _test_simple_bg(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    if  "[1]" in output1 and "hello" in output2:
...
def _test_command_terminate(comment_file_path, student_dir, command_wait=0.01):
  start_test(comment_file_path, "A sleep command terminates")
...
def _test_responsive_shell(comment_file_path, student_dir, command_wait=0.01):
...
def _test_multiple_bg(comment_file_path, student_dir, command_wait=0.01):
...
def _test_single_ps(comment_file_path, student_dir, command_wait=0.01):
...
def _test_multiple_ps(comment_file_path, student_dir, command_wait=0.01):
...
def _test_bg_completes(comment_file_path, student_dir, command_wait=0.01, length_cutoff=60):
...
def _test_bg_terminates(comment_file_path, student_dir, command_wait=0.01, length_cutoff=70):
...
def _test_count_reset(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    write(p,"sleep 0.5 &")
    output = read_stdout(p)
    if "[1]" in output and "[3]" not in output:
      finish(comment_file_path, "OK")
    else:
      finish(comment_file_path, "NOT OK")
  except Exception as e:
...
def _test_exceed_limits(comment_file_path, student_dir, command_wait=0.01):
...
def _test_bg_pipes(comment_file_path, student_dir, command_wait=0.01, length_cutoff=35):
...
def test_bg_suite(comment_file_path, student_dir):
  ...
  start_with_timeout(_test_simple_bg, comment_file_path, student_dir)
  start_with_timeout(_test_responsive_shell, comment_file_path, student_dir)
  start_with_timeout(_test_multiple_bg, comment_file_path, student_dir)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Sample ps runs")
  start_with_timeout(_test_single_ps, comment_file_path, student_dir)
  start_with_timeout(_test_multiple_ps, comment_file_path, student_dir)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Background jobs finish correctly")
  start_with_timeout(_test_bg_completes, comment_file_path, student_dir, timeout=2)
  start_with_timeout(_test_bg_terminates, comment_file_path, student_dir)
  end_suite(comment_file_path)
  
  start_suite(comment_file_path, "bg edge cases")
  start_with_timeout(_test_count_reset, comment_file_path, student_dir, timeout=3)
  start_with_timeout(_test_exceed_limits, comment_file_path, student_dir)
  end_suite(comment_file_path)
