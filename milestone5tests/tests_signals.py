...
def _test_survive(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    sleep(command_wait)
    read_stdout(p)   # The CTRL+C line should be mysh$ 
    write(p, "echo hi")
    sleep(command_wait)
...
def _test_terminate(comment_file_path, student_dir, command_wait=0.01):
...
def _test_terminate_signal(comment_file_path, student_dir, command_wait=0.01):
...
def _test_invalid_pid(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    if "ERROR: The process does not exist" not in output1:
      finish(comment_file_path, "NOT OK")
      return 
    
    if "ERROR: Builtin failed: kill" not in output2:
      finish(comment_file_path, "NOT OK")
      return 
...
def _test_invalid_signal(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    if "ERROR: Invalid signal specified" not in error1:
      finish(comment_file_path, "NOT OK")
      return 
    
    if "ERROR: Builtin failed: kill" not in error2:
      finish(comment_file_path, "NOT OK")
      return 
...
def _test_terminate_variable(comment_file_path, student_dir, command_wait=0.01):
