...
def _test_echo_plain(comment_file_path, student_dir, command_wait=0.01):
  start_test(comment_file_path, "Non-existing variable displays as plain text")
  try:
    p = start('./mysh')
    write(p,"echo $x")
    output = read_stdout(p)
...
def test_variables_suite(comment_file_path, student_dir):
  ...
  start_with_timeout(_test_access, comment_file_path)
  start_with_timeout(_test_access_v2, comment_file_path)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Variable integration with other commands")
  start_with_timeout(_test_declare, comment_file_path)
  start_with_timeout(_test_two_accesses, comment_file_path)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Custom variable accesses")
  start_with_timeout(_test_multiline_access, comment_file_path)
  start_with_timeout(_test_multiline_access_v2, comment_file_path)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Echo without variables displays plain text")
  start_with_timeout(_test_echo_novars, comment_file_path)
  start_with_timeout(_test_echo_plain, comment_file_path)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Variable values can be redefined")
  start_with_timeout(_test_redefine, comment_file_path)
  start_with_timeout(_test_redefine_v2, comment_file_path)
  end_suite(comment_file_path)

  start_suite(comment_file_path, "Variable formatting edge cases")
  start_with_timeout(_test_multiple_equals, comment_file_path)
  start_with_timeout(_test_no_spaces, comment_file_path)
  end_suite(comment_file_path)
